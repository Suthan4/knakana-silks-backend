...
console.log("pickupWarehouse",pickupWarehouse);
  ...
  // ✅ CREATE ORDER FIRST
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ✅ FIX: Shiprocket payload — sub_total must equal order.total so the
    //    shipping label ORDER TOTAL matches the tax invoice TOTAL exactly.
    //
    //    Root cause of the mismatch:
    //      • Invoice showed  Rs. 1686.51  (subtotal 1500 + shipping 106.20
    //                                      + CGST 40.16 + SGST 40.16)
    //      • Shipping label showed 1500 INR because we were passing
    //        sub_total: Number(order.subtotal)  ← only the product subtotal
    //
    //    Fix: pass sub_total: Number(order.total)  ← the full invoice total.
    //
    //    Additional fixes in this block:
    //      1. billing_* fields now use billingAddress (not shippingAddress).
    //      2. shipping_is_billing respects whether the two address IDs differ.
    //      3. selling_price per line-item uses the same price as the invoice
    //         (variant.price ?? product.sellingPrice) so individual line
    //         amounts are consistent.
    //      4. tax: 0 per line-item — GST is already included in sub_total.
    // ─────────────────────────────────────────────────────────────────────────
    const shiprocketPayload = {
      order_id: String(order.orderNumber),
      order_date: new Date().toISOString(),

      pickup_location: "work", // must match pickup location name in Shiprocket

      // ✅ FIX 1: use billingAddress for billing fields
      ...
      // ✅ FIX 2: only mark shipping_is_billing when address IDs actually match
      shipping_is_billing: notes.shippingAddressId === notes.billingAddressId,
      ...
      // ✅ FIX 3: use same price source as invoice
      order_items: orderItems.map((item) => {
        ...
        return {
          ...
          discount: 0,
          // ✅ FIX 4: tax 0 per line — GST is captured at order level in sub_total
          tax: 0,
      ...
      // ✅ KEY FIX: use order.total (not order.subtotal) so the shipping label
      //    ORDER TOTAL = subtotal + shippingCost + CGST + SGST = invoice TOTAL
