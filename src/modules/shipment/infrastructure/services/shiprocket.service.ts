...
export class ShiprocketService {
  ...
  /**
   * Shiprocket queues manifest generation. /manifests/generate often replies
   * with { message: "Manifest not generated", check_ids: [...] } instead of
   * a manifest_url while the PDF is still being built. Poll /manifests/print
   * (using the Shiprocket ORDER ids, not shipment ids) until the url shows up.
   */
  async printManifest(orderIds: number[]) {
    try {
      const response = await this.api.post("/manifests/print", {
        order_ids: orderIds,
      });
      return response.data;
    } catch (error: any) {
      console.error("Manifest print failed:", error.response?.data || error);
