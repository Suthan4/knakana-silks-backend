...
import { AsyncUtil } from "@/shared/utils/index.js";
 ...
 awbCode: awbResponse.response.data.awb_code,              // ✅ ADDED
  ...
  async generateManifest(orderId: string) {
    ...
    if (!shipment.shiprocketOrderId) {
      throw new Error("Shiprocket order id missing for this shipment");
    }
    ...
    // Fast path: Shiprocket returned the PDF url immediately.
    if (manifestResponse?.manifest_url) {
      return manifestResponse;
    }

    // Slow path: Shiprocket queued the job (typical response here is
    // { message: "Manifest not generated", check_ids: [...] }). Poll
    // /manifests/print with the Shiprocket ORDER id until the url is ready.
    const shiprocketOrderId = parseInt(shipment.shiprocketOrderId);
    const maxAttempts = 5;
    const delayMs = 2000;

    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      await AsyncUtil.sleep(delayMs);

      const printResponse = await this.shiprocketRepository.printManifest([
        shiprocketOrderId,
      ]);

      if (printResponse?.manifest_url) {
        return printResponse;
      }
    }

    // Still not ready after polling — surface a clear, honest error instead
    // of a false "success".
    throw new Error(
      "Manifest is still being generated by Shiprocket. Please try downloading it again in a minute."
    );
  }
